...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n):
        if k == 0:
            ...
            adding_height_top = rect.get_upper_left_corner().y - (delta * k)
            adding_height_bot = rect.get_lower_left_corner().y + (delta * k)
            adding_width_left = rect.get_upper_left_corner().x - (delta * k)
            adding_width_right = rect.get_upper_right_corner().x + (delta * k)
            new_rect = rg.Rectangle(rg.Point(adding_width_left, adding_height_top), rg.Point(adding_width_right, adding_height_bot))
