...
def problem0c(circle, n, window):
    ...
    for k in range(n):
        more_circles = rg.Circle(rg.Point(circle.center.x + (2 * circle.radius * (k + 1)), circle.center.y), circle.radius)
        more_circles.attach_to(window)
        window.render(0.5)
